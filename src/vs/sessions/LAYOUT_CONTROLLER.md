...
The abstract `BaseLayoutController` owns the platform-agnostic mechanics (panel, editor working sets, persistence, multi-session suppression). `LayoutController` (desktop) adds auxiliary bar management; `MobileLayoutController` omits it. `contrib/layout/browser/sessions.layout.contribution.ts` contributes the correct controller per platform (and registers the experimental responsive-sidebar setting); it is imported from `sessions.desktop.main.ts`.
