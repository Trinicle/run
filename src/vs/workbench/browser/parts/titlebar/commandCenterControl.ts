...
class CommandCenterCenterViewItem extends BaseActionViewItem {
			...
			const toolbar = this._instaService.createInstance(WorkbenchToolBar, container, {
				...
				actionViewItemProvider: (action, options) => {
					...
					return this._instaService.createInstance(class CommandCenterQuickPickItem extends BaseActionViewItem {
						...
						override render(container: HTMLElement): void {
							super.render(container);
							container.classList.add('command-center-folder-label');
							container.setAttribute('aria-label', this._getLabel());
						...
						override onClick(): void {
							// Folder label is not a search launcher; Quick Open lives on the sidebar header.
						}
						...
						private _getLabel(): string {
							return that._windowTitle.workspaceName;
						}
					});
				}
			});
	...
	protected override getTooltip() {
		...
		const title = this._windowTitle.workspaceName || this._windowTitle.value;
		return title;
	}
}
