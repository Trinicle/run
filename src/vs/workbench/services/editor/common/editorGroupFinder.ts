...
function editorNeverOpensModal(editor: EditorInput | IUntypedEditorInput | undefined): boolean {
...
function handleGroupResult(group: IEditorGroup, editor: EditorInputWithOptions | IUntypedEditorInput, preferredGroup: PreferredGroup | undefined, editorGroupService: IEditorGroupsService, configurationService: IConfigurationService): FindGroupResult {
	...
	// case the user preference wins. `NeverModal` always wins over modal routing.
	const requiresModal = !neverModal && editorInput instanceof EditorInput && editorInput.hasCapability(EditorInputCapabilities.RequiresModal) && modalEditorMode !== 'off';
	if (neverModal && modalEditorPart?.groups.some(modalGroup => modalGroup.id === group.id)) {
...
function doFindGroup(input: EditorInputWithOptions | IUntypedEditorInput, preferredGroup: PreferredGroup | undefined, editorGroupService: IEditorGroupsService, configurationService: IConfigurationService): Promise<IEditorGroup> | IEditorGroup {
	...
	const neverModal = editorNeverOpensModal(editor);
	...
	if (!neverModal && isEditorInput(editor) && editor.hasCapability(EditorInputCapabilities.RequiresModal) && configurationService.getValue<UseModalEditorMode>(USE_MODAL_EDITOR_SETTING) !== 'off') {
	...
	else if (!neverModal && preferredGroup === MODAL_GROUP && configurationService.getValue<UseModalEditorMode>(USE_MODAL_EDITOR_SETTING) !== 'off') {
		group = editorGroupService.createModalEditorPart(options?.modal)
			.then(part => part.activeGroup);
	}
	...
	if (!group && !neverModal && configurationService.getValue<UseModalEditorMode>(USE_MODAL_EDITOR_SETTING) === 'all') {
		group = editorGroupService.createModalEditorPart(options?.modal)
			.then(part => part.activeGroup);
	}
